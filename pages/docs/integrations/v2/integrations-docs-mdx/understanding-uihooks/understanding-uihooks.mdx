...
A UiHook is the main component of a ZEIT Integration. In [the Getting Started section](#getting-started), we used a utility from the `@zeit/integration-utils` module to [create a UiHook](#getting-started/creating-a-uihook).
...
- "Installation": A single instance where a user installed an Integration to an account (user or a team).
...
For more examples of Integration implementation including the above example and others in more languages, such as PHP, Python, Go and Rust, [see the Integration Gallery repository](https://github.com/zeit/integrations-gallery/tree/master/counter-micro).

## HTTP Payload

The payload used in the UiHook receives a JSON payload with different pieces of information regarding the Integration installation, those being:
...
- `teamId` - The `teamId` if the Integration is installed to a team.
- `projectId` - The [`projectId`](#creating-user-interfaces/information/project-information) if the Integration installation is inside a project. (This will be `null` if the Integration installation is from within an account and not a project).
- `token` - A short-lived [token](#creating-user-interfaces/tokens) which has authorization for the installed account.
- `installationUrl` - The URL of the current Integration installation. (You can use this to redirect a user to another site using `installationUrl` as the redirect URL)
